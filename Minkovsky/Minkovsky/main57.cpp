...
bool CheckLowLeft(const TypeVertex& firstVertex, const TypeVertex& secondVertex) {
	return ((firstVertex.y < secondVertex.y) || ((firstVertex.y == secondVertex.y) && (firstVertex.x < secondVertex.x)));
}

void GetMinkovskySum(vector<TypeVertex>& firstPolygon, vector<TypeVertex>& secondPolygon, vector<TypeVertex>* sum) {
	int firstIndex = 0;
	int secondIndex = 0;
	for (int i = 0; i < firstPolygon.size(); ++i) {
		if (CheckLowLeft(firstPolygon[i], firstPolygon[firstIndex])) {
			firstIndex = i;
		}
	}
	for (int i = 0; i < secondPolygon.size(); ++i) {
		if (CheckLowLeft(secondPolygon[i], secondPolygon[secondIndex])) {
			secondIndex = i;
		}
	}
	...
	do {	
		...
		if (crossProduct - EPSILON > 0) {
			++secondIndex;		
		} else if (crossProduct + EPSILON < 0) {
			++firstIndex;
		} else {
			++firstIndex;
			++secondIndex;
...
void GetMinkovskyDiff(vector<TypeVertex>& firstPolygon, vector<TypeVertex>& secondPolygon, vector<TypeVertex>* diff) {
	for (int i = 0; i < invertedSecondPolygon.size(); ++i) {
		secondPolygon[i].x = -secondPolygon[i].x;
		secondPolygon[i].y = -secondPolygon[i].y;
	}
	GetMinkovskySum(firstPolygon, secondPolygon, diff);
} 

bool CheckIntersection(vector<TypeVertex>& firstPolygon, vector<TypeVertex>& secondPolygon) {
