...
bool CheckVertexInclusion(const vector<TypeVertex>& polygon, const TypeVertex& vertex) {
	if (IsRotationCounterClockwise(polygon[0], polygon[polygon.size() - 1], vertex) || IsRotationClockwise(polygon[0], polygon[1], vertex)) {
		return false;
	}
	...
	while (rightIndex - leftIndex > 1) {
		...
		if (IsRotationClockwise(polygon[0], polygon[midIndex], vertex)) {
			rightIndex = midIndex;
		} else {
			leftIndex = midIndex;
		}
